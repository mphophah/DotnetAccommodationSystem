...
namespace AMS.Controllers
{
    ...
    public class SalaryController : Controller
    {
        ...
        public async Task<ActionResult> Create(SalaryVM model)
        {
        ...
        {
            var invoiceApp = await _unitOfWork.Salarys.Find(q => q.Id == id,
            includes: q => q.Include(x => x.Stuff));

            var model = _mapper.Map<EditSalaryVM>(invoiceApp);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> Edit(EditSalaryVM model)
        {
            try
            {
                ...
                var invoice = _mapper.Map<Salary>(model);
                invoice.isDeleted = "no";
                _unitOfWork.Salarys.Update(invoice);
                await _unitOfWork.Save();
        ...
        public async Task<ActionResult> Delete(int id)
        {
            try
            {
                var invoices = await _unitOfWork.Salarys.Find(expression: q => q.Id == id);
                if (invoices == null)
                {
                    return NotFound();
                }
                ...
                _unitOfWork.Salarys.Update(invoices);
                await _unitOfWork.Save();
        ...
        public IActionResult Payslip(int id)
        {
            ...
            var data = _db.Salarys.Include(x => x.Stuff).Where(x => x.Id == id).ToList();
